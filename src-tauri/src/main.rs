...
fn main() {
    tauri::Builder::default()
        .setup(|app| {
            ...
            std::thread::sleep(std::time::Duration::from_millis(200)); // Create a WgpuState (containing the device, instance, adapter etc.)
            ...
            async_runtime::spawn(async move {
                ...
                while let Ok(buffer) = rx.recv() {
                    ...
                    // Attempt to get the surface texture, but handle potential errors during resize
                    // Attempt to get the surface texture, but handle potential errors during resize
                    let output = match wgpu_state.surface.get_current_texture() {
                        ...
                        Err(wgpu::SurfaceError::Outdated | wgpu::SurfaceError::Lost) => {
                            // Surface needs reconfiguration, reconfigure and try again
                    ...
                    output.present();
                    // output is dropped here automatically
